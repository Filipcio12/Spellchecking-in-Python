...
def compression_ratio(s, compressed_s):
    binary_s = "".join(format(ord(i), "08b") for i in s)

    return len(binary_s) / len(compressed_s)


def main():
    string = "I love data structures"
    codes = huffman(string)
    compressed_s = convert_chars(string, codes)
    ratio = compression_ratio(string, compressed_s)

    print(f"codes:  {codes}")
    print(f"string: {compressed_s}")
    print(f"ratio:  {ratio}")
